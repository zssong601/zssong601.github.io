...
export default function ReadinessCheck() {
  ...
  return <div className="readiness-check">{!complete ? <div className="survey-questions">{questions.map((question, questionIndex) => <fieldset key={question.id}><legend><span>0{questionIndex + 1}</span>{question.prompt}</legend><div>{question.options.map((option) => <label key={option}><input type="radio" name={question.id} value={option} checked={answers[question.id] === option} onChange={() => setAnswers({ ...answers, [question.id]: option })} /><span>{option}</span></label>)}</div></fieldset>)}</div> : <div className="survey-result"><p className="eyebrow">A PRACTICAL PLACE TO BEGIN</p><h2>{result}</h2><p>This result is not stored or sent anywhere. It is simply a way to choose a focused first conversation.</p><div className="survey-actions"><a className="button button-dark" href="https://calendly.com/zssong601/15min" target="_blank" rel="noreferrer">Book a 15-minute fit call <span>↗</span></a><button className="reset-button" onClick={() => setAnswers({})}>Start again</button></div></div>}</div>;
}
